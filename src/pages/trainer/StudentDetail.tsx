...
const PageTitle = styled.h1`font-size: 22px; font-weight: 600; margin-bottom: 4px;`;
const SubInfo = styled.p`font-size: 13px; color: ${({ theme }) => theme.text.secondary}; margin-bottom: 24px;`;
...
const DayLabel = styled.span`font-weight: 500; min-width: 80px;`;
const WorkoutName = styled.span`color: ${({ theme }) => theme.text.secondary};`;

const DAYS = ["Domingo", "Segunda", "Terça", "Quarta", "Quinta", "Sexta", "Sábado"];

interface WorkoutType { id: string; name: string; }
interface StudentWorkoutDay { id: string; dayOfWeek: number; workoutType: WorkoutType; }
interface Student {
  id: string; name: string; email?: string;
  accessCode: string; status: string;
}
...
const StudentDetail: React.FC = () => {
  ...
  if (!student) return <div style={{ padding: 32, color: "#888" }}>Carregando...</div>;
  ...
  return (
    <div>
      ...
      <CodeBox>
        ...
        <CodeLabel style={{ marginTop: 10 }}>Definir código personalizado (PIN ou palavra)</CodeLabel>
                ...
                {assigned ? (
                  <>
                    ...
                    <button
                      ...
                      style={{ fontSize: 12, color: "#dc2626", padding: "2px 8px" }}
                    >
                      Remover
                    </button>
                  </>
